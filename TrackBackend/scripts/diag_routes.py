    ...
    for i, branch in enumerate(branches):
        if isinstance(branch, dict):
            ...
            if len(branch) > 0 and isinstance(branch[0], dict):
                ...
                print(f'    Looks like coordinate arrays, first has {len(branch[0])} items')
    print()
                    ...
                    for pt in segment:
                        if isinstance(pt, list) and len(pt) >= 2:
                            ...
                            if 40.695 < lat < 40.715 and -73.820 < lon < -73.795:
                                print(f'  Route {r} branch {i} seg {j}: lat={lat}, lon={lon} NEAR SUTPHIN')
                                break
            ...
            if isinstance(coords, list):
                for j, pt in enumerate(coords):
                    if isinstance(pt, list) and len(pt) >= 2:
                        lat, lon = pt[0], pt[1]
                        if 40.695 < lat < 40.715 and -73.820 < lon < -73.795:
                            print(f'  Route {r} branch {i}: coord {j}: lat={lat}, lon={lon} NEAR SUTPHIN')
                            break
