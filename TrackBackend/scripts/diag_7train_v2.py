...
from app.services.mapping.corridor_pipeline import (
    _group_and_merge_trunks, _compute_corridor_offsets,
    TRUNK_GROUPS, ROUTE_TO_TRUNK, _to_wgs84,
    CORRIDOR_DETECT_DIST, CORRIDOR_ALIGN_MIN,
)
...
for l in lines:
    ...
    if raws:
        ov.append(SubwayLineOverlay(route_id=l, color_hex='', polylines=[encode_polyline(r) for r in raws]))
...
for seg_idx in sorted(offs.get(trunk_id, {}).keys()):
    ...
    if offset_lats:
        ...
        print(f"  Geographic extent: ({min_lat:.5f},{min_lon:.5f}) to ({max_lat:.5f},{max_lon:.5f})")
