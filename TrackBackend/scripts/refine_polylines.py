...
BUNDLE_PATH = (
    Path(__file__).parent.parent.parent / "Track" / "Data" / "subway_bundle.json"
)
...
def angle_at(p1, p2, p3):
    ...
    v1x = p1[0] - p2[0]
    v1y = (p1[1] - p2[1]) * COS_NYC
    v2x = p3[0] - p2[0]
    v2y = (p3[1] - p2[1]) * COS_NYC
    dot = v1x * v2x + v1y * v2y
...
class WayGraph:
    ...
    def find_path(self, start_lat, start_lon, end_lat, end_lon, max_total_m=15000):
        ...
        while open_set:
            _f, g, current, path = heapq.heappop(open_set)
            ...
            for neighbor, dist in self.adj.get(current, []):
                ...
                heapq.heappush(
                    open_set, (new_g + h, new_g, neighbor, [*path, neighbor])
                )
...
def load_osm(osm_path):
    ...
    print(
        f"  {len(filtered_ways)} mainline ways, {index.count} segments, {len(graph_nodes)} graph nodes"
    )
    return index, graph
...
def step1_snap(bundle, index):
    ...
    for rid in sorted(routes):
        for _bi, branch in enumerate(routes[rid]):
    ...
    print(
        f"  Snapped {snapped_count}/{total} points (avg within {MAX_SNAP_DISTANCE_M}m)"
    )
    return bundle
            ...
            for i in range(1, len(branch)):
                ...
                if gap > GAP_THRESHOLD_M:
                    ...
                    if path and len(path) >= 3:
                        ...
                        added += len(fill_pts)
                        print(
                            f"    Route {rid} br{bi}: filled {gap:.0f}m gap with {len(fill_pts)} pts (A*)"
                        )
...
def step2b_dedup_branches(bundle):
    ...
    for rid in sorted(routes):
        ...
        def add_to_grid(branch, grid=grid):
            for pt in branch:
                lc, nc = cell_key(pt["lat"], pt["lon"])
                grid.add(pack(lc, nc))

        def is_covered(lat, lon, grid=grid):
        ...
        def is_new_terminal(lat, lon, kept_endpoints=kept_endpoints):
        ...
        def extract_stubs(branch, min_pts=MIN_STUB_PTS):
            ...
            for rs, re in runs:
                ...
                if (
                    is_near(branch[rs]["lat"], branch[rs]["lon"])
                    and is_near(branch[mid]["lat"], branch[mid]["lon"])
                    and is_near(branch[re]["lat"], branch[re]["lon"])
                ):
                    continue
        ...
        for _orig_idx, branch in indexed[1:]:
            if len(branch) < 2:
                continue
    ...
    print(
        f"  {branches_before} → {branches_after} branches, {total_before} → {total_after} points"
    )
    return bundle
...
def step3_remove_artifacts(bundle):
    ...
    for rid in sorted(routes):
        for bi, branch in enumerate(routes[rid]):
            ...
            for i in range(1, len(branch)):
                if (
                    branch[i]["lat"] != cleaned[-1]["lat"]
                    or branch[i]["lon"] != cleaned[-1]["lon"]
                ):
...
def step4_smooth(bundle):
    ...
    print(
        f"  {total_before} → {total_after} points (Catmull-Rom, {SMOOTH_SEGMENTS} segs/curve)"
    )
    return bundle
