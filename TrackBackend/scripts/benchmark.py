...
async def _fetch_track(lat: float, lon: float, radius: int) -> list[dict]:
    ...
    try:
        ...
        return [
            a
            for a in r.json()
            if a.get("mode") == "bus" and a.get("minutes_away", 99) < 60
        ]
...
async def _run_location(loc: dict, radius: int) -> dict:
    ...
    # Merge stop IDs
    track_stop_ids = list(
        {_canon(a.get("stop_id", "")) for a in track_arrivals if a.get("stop_id")}
    )
...
def _print_location(res: dict) -> None:
    ...
    print(
        f"  │  Track arrivals: {res['track_n']}  │  SIRI arrivals: {res['siri_n']}  │  Matched: {res['matched']}"
    )
    ...
    p90 = (
        sorted(abs(d) for d in diffs)[int(len(diffs) * 0.9)]
        if len(diffs) >= 3
        else max(abs(d) for d in diffs)
    )
    ...
    for r in notable:
        ...
        print(
            f"  │  {tag}{r['route']:<7} stop={r['stop']:<10} raw={r['raw']:>3}m  ml={r['ml']:>3}m  Δ={r['diff']:+d}m"
        )
    print("  └" + "─" * 60)
...
def _print_aggregate(all_results: list[dict]) -> None:
    ...
    bias_label = (
        "model inflates"
        if mean_signed > 0.3
        else "model deflates" if mean_signed < -0.3 else "well-calibrated"
    )
    ...
    print()
    print(
        f"    0 min   : {zeroes:4d} arrivals  ({zeroes/len(diffs)*100:.0f}%)  model = MTA exactly"
    )
    print(
        f"   +1 min   : {pos1:4d} arrivals  ({pos1/len(diffs)*100:.0f}%)  small inflation"
    )
    print(
        f"   +2 min   : {pos2:4d} arrivals  ({pos2/len(diffs)*100:.0f}%)  moderate inflation"
    )
    print(
        f"  ≥+3 min   : {pos3p:4d} arrivals  ({pos3p/len(diffs)*100:.0f}%)  alert/recency correction active"
    )
    print(
        f"  Early     : {neg:4d} arrivals  ({neg_pct:.0f}%)  model predicts early (recency)"
    )
    print()
    ...
    print(
        "  ℹ  ≥+3   = recency correction (stop logging late trips) or active alert boost."
    )
    print(
        "  ℹ  Early = recency correction on routes that habitually run ahead of schedule."
    )
    print("━" * 72)
...
async def main() -> None:
    ...
    parser.add_argument(
        "--radius", type=int, default=600, help="Search radius in meters"
    )
    parser.add_argument(
        "--concurrency", type=int, default=3, help="Parallel location fetches"
    )
