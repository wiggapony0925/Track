...
from app.utils.polyline_utils import encode_polyline, decode_polyline
...
for lat, lon in test_coords:
    ...
    print(f"  ({lat:.6f}, {lon:.6f}) -> ({lat_final:.6f}, {lon_final:.6f})  err={err:.2f} m")
...
from app.services.mapping.shape_utils import pack_coords, unpack_coords, ShapePoint
from app.services.mapping.subway_shapes import _load_shapes
...
for shape_id, buf in shapes.items():
    ...
    if len(coords) >= 50:
        ...
        decoded = decode_polyline(encoded)
        errors = []
        for (lat32, lon32), (lat_dec, lon_dec) in zip(coords[:50], decoded):
            errors.append(calc_error(lat32, lon32, lat_dec, lon_dec))
        print(f"  Shape {shape_id}: {len(coords)} pts")
        print(f"  float32->encode->decode max error: {max(errors):.2f} m, avg: {sum(errors)/len(errors):.2f} m")
        ...
        print(f"  (Note: float32 error from original GTFS is already baked in)")
        break
