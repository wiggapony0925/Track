...
async def fetch_all_route_tags():
    ...
    print(f"🚀 Starting scrape of valid Route IDs...")
                ...
                if data.get('code') == 200:
                    ...
                    for route in routes:
                        ...
                        if short_name and official_id:
                            ...
                            elif sn_upper.startswith("B") and not sn_upper.startswith("BX"):
                                # Ensure it's not a Manhattan or Bronx route misidentified
                                categorized_map["Brooklyn"][short_name] = official_id
                            elif sn_upper.startswith("M") and not sn_upper.startswith(("BM", "BXM")):
                                categorized_map["Manhattan"][short_name] = official_id
                            elif sn_upper.startswith("Q") and not sn_upper.startswith("QM"):
                                categorized_map["Queens"][short_name] = official_id
                            elif sn_upper.startswith("S") and not sn_upper.startswith("SIM"):
                                categorized_map["Staten Island"][short_name] = official_id
                            else:
