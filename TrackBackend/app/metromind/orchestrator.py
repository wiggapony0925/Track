...
async def run_turn(
    *,
    ...
    """Execute a single user turn and return ``(reply_text, tool_names_used)``."""
    settings = get_metromind_settings()
    system_prompt = render_system_prompt(context)
    messages = _format_history(
        system_prompt, history, user_message, window=settings.history_window
    )
    tools = tool_schemas()
    used_tools: list[str] = []
    ...
    *,
    ...
    try:
        for iteration in range(settings.max_tool_iterations + 1):
            ...
            async for chunk in client.stream(messages=messages, tools=tools):
            ...
            if not tool_calls_buf:
                # Done — no tool calls, stream ended.
                yield SSEDoneEvent(tool_calls=used_tools).model_dump()
                return
...
_UI_PAYLOAD_TOOLS = {"plan_route", "search_stations"}
