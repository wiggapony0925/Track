...
class _AlertState:
    """Module-level alert cache state."""

    boost_by_route: dict[str, float] = dataclasses.field(default_factory=dict)
...
def get_alert_boost(route_id: str) -> float:
    ...
    if "_" in key:
        key = key.split("_")[-1]
    return _alert_state.boost_by_route.get(key, 0.0)
...
async def maybe_refresh() -> None:
    ...
    """
    if _time.time() - _alert_state.last_refresh < _REFRESH_INTERVAL:
        return
    ...
    try:
        ...
        for alert in alerts:
            ...
            boost = (
                _SEVERE_BOOST
                if sev == "severe"
                else (_WARNING_BOOST if sev == "warning" else 0.0)
            )
            if boost == 0.0:
                continue
        ...
        TrackLogger.info(
            "[ALERTS] Refresh timed out after 3s — using stale index", tag="ML"
        )
        _alert_state.last_refresh = (
            _time.time()
        )  # back off, don't hammer on repeated timeouts
    except (httpx.HTTPError, OSError, KeyError, ValueError) as exc:
        TrackLogger.info(
            f"[ALERTS] Refresh failed ({exc}) — using stale index", tag="ML"
        )
        _alert_state.last_refresh = (
            _time.time()
        )  # back off, don't hammer on repeated errors
