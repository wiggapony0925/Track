...
class ElevatorStatus(BaseModel):
    ...
    station: str = Field(..., description="Station name where the equipment is located.")
    equipment_type: str = Field(..., description="Type of equipment: 'EL' (elevator) or 'ES' (escalator).")
    description: str = Field(..., description="Human-readable description of the outage.")
    outage_since: str | None = Field(None, description="When the outage began (ISO 8601 or descriptive text).")


class BusRoute(BaseModel):
    """A normalized bus route from the OBA API."""

    id: str = Field(..., description="Fully-qualified OBA route ID (e.g. 'MTA NYCT_B63').")
    short_name: str = Field(..., description="Short route name displayed on the bus (e.g. 'B63').")
    long_name: str = Field(..., description="Full route name (e.g. 'Atlantic Av / 5 Av').")
    color: str = Field(..., description="Route brand colour as a hex string (e.g. '00AEEF').")
    description: str = Field(..., description="Route description or service area summary.")
...
class BusStop(BaseModel):
    ...
    direction: str | None = Field(None, description="Cardinal direction the bus travels at this stop (e.g. 'N', 'SW').")
    route_ids: list[str] = Field([], description="Fully-qualified route IDs served by this stop.")
...
class BusArrival(BaseModel):
    ...
    route_id: str = Field(..., description="Fully-qualified bus route ID (e.g. 'MTA NYCT_B63').")
    ...
    status_text: str = Field(..., description="Display text combining destination and ETA.")
    status: str = Field("Live", description="Status indicator: 'Live', 'Scheduled', or 'Approaching'.")
    expected_arrival: datetime | None = Field(None, description="Predicted arrival time (UTC ISO 8601).")
    aimed_arrival: datetime | None = Field(None, description="Scheduled arrival time from SIRI AimedArrivalTime.")
    schedule_deviation_s: int | None = Field(None, description="Delay in seconds (positive = late, negative = early).")
    distance_meters: float | None = Field(None, description="Distance from the vehicle to this stop in meters.")
    bearing: float | None = Field(None, description="Vehicle heading in degrees (0-360).")
    direction_ref: int | None = Field(None, description="SIRI direction reference: 0 or 1.")
    destination_name: str | None = Field(None, description="Trip destination (e.g. 'JAMAICA via BREWER BL').")
    is_realtime: bool = Field(True, description="False when position is estimated from schedule, not live GPS.")
...
class NearbyTransitArrival(BaseModel):
    ...
    destination: str | None = Field(None, description="Terminal station name (e.g. 'Wakefield-241 St').")
    minutes_away: int = Field(..., description="Minutes until arrival.")
    arrival_ts: int | None = Field(None, description="Arrival time as Unix epoch seconds.")
    status: str = Field("On Time", description="Current status: 'On Time', 'Delayed', etc.")
    ...
    trip_id: str | None = Field(None, description="GTFS trip ID for this vehicle run.")
    distance_m: float | None = Field(None, description="Haversine distance from user to stop in meters.")
    is_real_time: bool = Field(False, description="True when backed by live GTFS-RT or SIRI data.")
...
class DirectionArrivals(BaseModel):
    ...
    direction_label: str = Field("", description="Human-readable label (e.g. 'Northbound', 'Eastbound').")
    arrivals: list[NearbyTransitArrival] = Field(..., description="Upcoming arrivals in this direction.")
...
class InlineAlert(BaseModel):
    ...
    affected_routes: list[str] = Field([], description="Route IDs affected by this alert.")
    alert_type: str | None = Field(None, description="MTA alert type (e.g. 'Delays', 'Planned - Suspended').")
    sort_order: int = Field(0, description="MTA severity rank (higher = more severe).")
...
class BusVehicle(BaseModel):
    ...
    bearing: float | None = Field(None, description="Vehicle heading in degrees (0-360).")
    next_stop: str | None = Field(None, description="ID of the next stop the vehicle will serve.")
    status_text: str | None = Field(None, description="Human-readable status text.")
    direction_ref: int | None = Field(None, description="SIRI direction reference: 0 or 1.")
    expected_arrival: datetime | None = Field(None, description="Expected arrival at the next stop (UTC ISO 8601).")
    onward_calls: list[BusArrival] = Field([], description="Downstream stop predictions for this vehicle.")
    is_realtime: bool = Field(True, description="False when position is interpolated from static schedule.")
    position_recorded_at: datetime | None = Field(None, description="When the GPS position was last recorded (RecordedAtTime). Stale > 3 min may indicate signal loss.")
...
class DirectionShape(BaseModel):
    ...
    headsign: str = Field("", description="Trip headsign (e.g. 'Manhattan', 'Far Rockaway').")
    polylines: list[str] = Field(..., description="Google-encoded polylines for this direction.")
    stops: list[BusStop] = Field(..., description="Ordered stops for this direction.")
    service_type: str | None = Field(None, description="Service pattern: 'express', 'local', 'mixed', or null.")
...
class RouteShape(BaseModel):
    ...
    polylines: list[str] = Field(..., description="Google-encoded polylines (all directions merged).")
    stops: list[BusStop] = Field(..., description="Ordered stop list (all directions merged).")
    directions: list[DirectionShape] = Field([], description="Per-direction shapes when available.")
    service_type: str | None = Field(None, description="Service pattern: 'express', 'local', 'mixed', or null.")
...
class SubwayLineOverlay(BaseModel):
    ...
    color_hex: str = Field(..., description="Route brand colour as hex (e.g. '#0039A6').")
    polylines: list[str] = Field(..., description="Google-encoded polylines for this line.")
...
class TrunkGroupPolylines(BaseModel):
    ...
    trunk_index: int = Field(..., description="Trunk group index (0-10) matching TRUNK_GROUPS order.")
    color_hex: str = Field(..., description="Trunk group colour as hex (e.g. '#0039A6').")
    route_ids: list[str] = Field(..., description="All route IDs in this trunk group (e.g. ['A', 'C', 'E']).")
    polylines: list[str] = Field(..., description="Google-encoded polylines (merged trunk + branches).")
    lane_offset: float = Field(0.0, description="Signed perpendicular offset in meters for low-zoom pixel separation.")
...
class SubwayStation(BaseModel):
    ...
    routes: list[str] = Field(..., description="Route IDs served at this station (e.g. ['A', 'C', 'E']).")
...
class ProcessedStation(BaseModel):
    ...
    name: str = Field(..., description="Human-readable station name.")
    is_transfer: bool = Field(..., description="True when station spans 2+ trunk groups (transfer hub).")
    positions: list[StopPosition] = Field(..., description="Per-route snapped positions for circle placement.")


class ProcessedStationsResponse(BaseModel):
    """Processed stations with offset-snapped positions."""

    stations: list[ProcessedStation] = Field(..., description="All processed subway stations.")
...
class CommuterRailLineOverlay(BaseModel):
    ...
    color_hex: str = Field(..., description="Line brand colour as hex (e.g. '#00985F').")
    polylines: list[str] = Field(..., description="Google-encoded polylines for this line.")
    mode: str = Field(..., description="Transit mode: 'lirr' or 'mnr'.")
    stops: list[CommuterRailStop] = Field([], description="Ordered stations along this line.")


class AllCommuterRailLinesResponse(BaseModel):
    """All LIRR and MNR line overlays for the full system map."""

    lines: list[CommuterRailLineOverlay] = Field(..., description="All commuter rail line overlays.")
...
class BusScheduleDeparture(BaseModel):
    ...
    departure_time: int = Field(..., description="Scheduled departure as Unix epoch seconds.")
...
class BusScheduleDirection(BaseModel):
    ...
    departures: list[BusScheduleDeparture] = Field(..., description="Upcoming scheduled departures.")
...
class BusScheduleResponse(BaseModel):
    ...
    directions: list[BusScheduleDirection] = Field(..., description="Departures grouped by direction.")
...
class WeatherResponse(BaseModel):
    ...
    symbol: str = Field("", description="SF Symbol name for the condition (e.g. 'sun.max.fill').")
    description: str = Field("", description="Human-readable condition (e.g. 'Partly Cloudy').")
    category: str = Field("", description="WeatherCondition category (e.g. 'clear', 'rain').")
    windspeed_kmh: float | None = Field(None, description="Wind speed in km/h.")
    is_day: bool = Field(True, description="True during daytime hours at the requested location.")


class ReloadModelResponse(BaseModel):
    """Result of the ML model hot-reload operation."""

    success: bool = Field(..., description="Whether the model was reloaded successfully.")
