...
from app.services.bus_client import get_stops, resolve_bus_id
...
class LegendaryTester:
    ...
    def print_banner(self):
        ...
        print(f"{Fore.WHITE}{Back.BLUE}{Style.BRIGHT}  SYSTEM INTEGRITY & FULL COVERAGE TEST  ")
        ...
        for line in lines:
            try:
                ...
                if res and len(res[1]) > 0:
                    print(f"  {Fore.GREEN}✅ {line}: {len(res[1])} stops, {len(res[0])} polylines")
                ...
                for attempt in range(retries + 1):
                    try:
                        ...
                        if stops and len(stops) > 0:
                            ...
                            return False, f"Bus {short_name}: {official_id} returned 0 stops"
                    except Exception as e:
    ...
    async def test_self_healing(self):
        ...
        try:
            ...
            if resolved and "_" in resolved:
                print(f"  {Fore.GREEN}✨ SUCCESS: Self-healed '{test_unknown}' -> '{resolved}' in {end-start:.2f}s")
                self.stats["healing"]["pass"] += 1
            else:
                print(f"  {Fore.RED}❌ FAILED: Could not self-heal unknown route '{test_unknown}'")
    ...
    async def test_commuter_rail(self):
        ...
        # 1. Color Check
        branches = ["Babylon Branch", "Harlem Line", "Hudson Line", "Ronkonkoma Branch", "Port Washington Branch"]
        ...
        try:
            ...
            print(f"  {Fore.GREEN}✅ LIRR Feed: Reachable ({len(larrivals)} active arrivals)")
            
            print(f"  {Fore.CYAN}📡 Checking Metro-North Real-time Feed...")
            marrivals = await mnr_arrivals()
            print(f"  {Fore.GREEN}✅ Metro-North Feed: Reachable ({len(marrivals)} active arrivals)")
    ...
    def print_summary(self):
        ...
        for category, data in self.stats.items():
            ...
            if percentage < 50: color = Fore.RED
            
            bar_len = 30
            passes = int((p / t) * bar_len)
            bar = f"{Fore.GREEN}{'█' * passes}{Fore.RED}{'█' * (bar_len - passes)}"
            
            print(f"{category.capitalize():<10} | {bar} | {color}{percentage:>6.1f}% ({p}/{t})")
