...
class TestPredictDelayEndpoint:
    ...
    def test_response_schema(self):
        ...
        data = _get({"minutes_away": 5, "route_id": "L", "hour": 14, "day_of_week": 3, "weather": "clear"})
    ...
    def test_weekend_rush_lower_than_weekday_rush(self):
        ...
        assert weekday >= weekend, f"Weekday rush ({weekday}) should be >= weekend ({weekend})"
    ...
    def test_rush_snow_worst_combination(self):
        ...
        rain = _get({**base, "weather": "rain"})["delay_factor"]
        snow = _get({**base, "weather": "snow"})["delay_factor"]
        assert snow >= rain >= clear, f"Expected snow({snow}) >= rain({rain}) >= clear({clear})"

    def test_zero_minutes(self):
        """0 minutes → 0 adjusted regardless of factor."""
        data = _get({"minutes_away": 0, "route_id": "L", "hour": 8, "day_of_week": 2, "weather": "snow"})
    ...
    def test_large_minutes_always_scaled_up(self):
        ...
        data = _get({"minutes_away": 30, "route_id": "F", "hour": 9, "day_of_week": 4, "weather": "snow"})
        assert data["adjusted_minutes"] >= data["original_minutes"]
        assert data["delay_factor"] >= 1.0
    ...
    def test_stop_id_param_accepted(self):
        ...
        data = _get({
            "minutes_away": 8, "route_id": "7", "hour": 8, "day_of_week": 2,
            "weather": "rain", "stop_id": "702S",
        })
        assert data["adjusted_minutes"] >= data["original_minutes"]
    ...
    def test_mode_param_accepted(self):
        ...
        data = _get({
            "minutes_away": 10, "route_id": "M15", "hour": 8, "day_of_week": 2,
            "weather": "clear", "mode": "bus",
        })
        assert data["adjusted_minutes"] >= data["original_minutes"]
        assert data["model_source"] in ("model", "heuristic", "cache")
    ...
    def test_schedule_deviation_accepted(self):
        ...
        data = _get({
            "minutes_away": 8, "route_id": "Q10", "hour": 8, "day_of_week": 2,
            "weather": "clear", "mode": "bus", "schedule_deviation_s": 180,
        })
        assert data["adjusted_minutes"] >= data["original_minutes"]
    ...
    def test_zero_deviation_uses_cache(self):
        ...
        data = _get({
            "minutes_away": 5, "route_id": "L", "hour": 14, "day_of_week": 3,
            "weather": "clear", "schedule_deviation_s": 0,
        })
    ...
    def test_original_minutes_echoed(self):
        ...
        data = _get({"minutes_away": 12, "route_id": "A", "hour": 10, "day_of_week": 3, "weather": "clear"})
