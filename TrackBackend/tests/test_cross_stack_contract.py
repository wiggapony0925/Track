...
class TestTrackArrivalContract:
    ...
    IOS_KEYS = [
        ...
        "destination", "minutes_away", "status", "trip_id",
        "arrival_ts", "is_cancelled", "stop_lat", "stop_lon",
    ]
...
class TestNearbyTransitArrivalContract:
    ...
    IOS_KEYS = [
        ...
        "stop_lat", "stop_lon", "stop_id", "vehicle_id",
        "trip_id", "distance_m", "is_real_time", "is_cancelled",
    ]
    ...
    def test_null_optionals(self):
        data = self._make(
            destination=None, arrival_ts=None, stop_lat=None,
            stop_lon=None, stop_id=None, vehicle_id=None,
            trip_id=None, distance_m=None,
        )
        for key in ["destination", "arrival_ts", "stop_lat", "stop_lon",
                     "stop_id", "vehicle_id", "trip_id", "distance_m"]:
            assert data[key] is None, f"Expected {key} to be None"

    def test_defaults_explicit(self):
        data = _json(NearbyTransitArrival(
...
class TestGroupedNearbyTransitContract:
    ...
    IOS_KEYS = [
        "route_id", "display_name", "mode", "color_hex",
    ...
    def _make_full(self) -> dict:
        arrival = NearbyTransitArrival(
            ...
            destination="Inwood-207 St", minutes_away=3,
            arrival_ts=int(time.time()) + 180,
            status="On Time", mode="subway",
            stop_lat=40.71, stop_lon=-74.0, stop_id="A28N",
        )
    ...
    def test_defaults_explicit(self):
        data = _json(GroupedNearbyTransit(
            route_id="X", display_name="X", mode="bus",
            directions=[],
        ))
    ...
    def test_nested_arrival_has_all_keys(self):
        ...
        _assert_keys(arrival, TestNearbyTransitArrivalContract.IOS_KEYS, "nested arrival")
...
class TestTransitAlertContract:
    ...
    IOS_KEYS = [
        "route_id", "title", "description", "severity", "mode",
        "updated_at", "affected_routes", "alert_type", "sort_order",
        "display_before_active", "active_period_end",
    ]
    ...
    def test_defaults_explicit(self):
        data = _json(TransitAlert(
            title="Test", description="test", severity="warning",
        ))
        assert data["mode"] == "subway"
    ...
    def test_null_optionals(self):
        data = self._make(
            route_id=None, updated_at=None, alert_type=None,
            display_before_active=None, active_period_end=None,
        )
        assert data["route_id"] is None
...
class TestElevatorStatusContract:
    ...
    def test_field_types(self):
        data = _json(ElevatorStatus(
            station="S", equipment_type="E", description="D", outage_since="2026-01-01",
        ))
        _assert_type(data, "station", str)
...
class TestBusArrivalContract:
    ...
    IOS_KEYS = [
        "route_id", "vehicle_id", "stop_id", "stop_name",
        "status_text", "status", "expected_arrival",
        "distance_meters", "bearing", "direction_ref",
        "destination_name", "is_realtime",
    ]
    ...
    def _make(self, **overrides) -> dict:
        defaults = dict(
            ...
            status="Live",
            expected_arrival=datetime(2026, 3, 23, 14, 30, 0, tzinfo=timezone.utc),
    ...
    def test_field_types(self):
        ...
        _assert_type(data, "status", str)
        _assert_type(data, "expected_arrival", str, nullable=True)  # ISO datetime string
    ...
    def test_defaults_explicit(self):
        data = _json(BusArrival(
            route_id="X", vehicle_id="V", stop_id="S", status_text="nearby",
        ))
...
class TestBusVehicleContract:
    ...
    IOS_KEYS = [
        "vehicle_id", "route_id", "lat", "lon", "bearing",
        "next_stop", "status_text", "direction_ref",
        "expected_arrival", "onward_calls", "is_realtime",
    ...
    def _make(self, **overrides) -> dict:
        defaults = dict(
            ...
            expected_arrival=datetime(2026, 3, 23, 14, 30, 0, tzinfo=timezone.utc),
            is_realtime=True,
            position_recorded_at=datetime(2026, 3, 23, 14, 28, 0, tzinfo=timezone.utc),
        )
        defaults.update(overrides)
    ...
    def test_onward_calls_nested_structure(self):
        ...
        vehicle = BusVehicle(
            vehicle_id="V1", route_id="R1", lat=40.0, lon=-74.0,
            onward_calls=[
                BusArrival(
                    route_id="R1", vehicle_id="V1", stop_id="S1",
                    status_text="next", direction_ref=0,
                ),
            ],
        )
        data = _json(vehicle)
    ...
    def test_defaults_explicit(self):
        data = _json(BusVehicle(
            vehicle_id="V", route_id="R", lat=0, lon=0,
        ))
        assert data["is_realtime"] is True
...
class TestBusRouteContract:
    ...
    def test_field_types(self):
        data = _json(BusRoute(
            id="X", short_name="SN", long_name="LN", color="C", description="D",
        ))
...
class TestRouteShapeContract:
    ...
    def test_defaults_explicit(self):
        data = _json(RouteShape(
            route_id="X", polylines=[], stops=[],
        ))
        assert data["directions"] == []
...
class TestSubwayLineOverlayContract:
    ...
    def test_all_ios_keys_present(self):
        data = _json(SubwayLineOverlay(
            route_id="A", color_hex="#0039A6",
            polylines=["_p~iF~ps|U_ulLnnqC"],
        ))
...
class TestTrunkGroupPolylinesContract:
    ...
    IOS_KEYS = [
        "trunk_index", "color_hex", "route_ids", "polylines",
        "lane_offset", "polyline_lane_offsets",
    ]

    def _make(self) -> dict:
        return _json(TrunkGroupPolylines(
            trunk_index=0,
            color_hex="#0039A6",
            route_ids=["A", "C", "E"],
    ...
    def test_defaults_explicit(self):
        data = _json(TrunkGroupPolylines(
            trunk_index=0, color_hex="#C", route_ids=[], polylines=[],
        ))
...
class TestAllSubwayLinesResponseContract:
    ...
    def test_all_ios_keys_present(self):
        data = _json(AllSubwayLinesResponse(
            lines=[SubwayLineOverlay(route_id="A", color_hex="#0039A6", polylines=["abc"])],
            trunk_polylines=[
                TrunkGroupPolylines(
                    trunk_index=0, color_hex="#0039A6",
                    route_ids=["A", "C", "E"], polylines=["abc"],
                ),
            ],
        ))
...
class TestSubwayStationContract:
    ...
    def test_all_ios_keys_present(self):
        data = _json(SubwayStation(
            id="A27", name="59 St-Columbus Circle",
            lat=40.768, lon=-73.981, routes=["A", "C", "B", "D", "1"],
        ))
        _assert_keys(data, self.IOS_KEYS, "SubwayStation")

    def test_field_types(self):
        data = _json(SubwayStation(
            id="X", name="N", lat=40.0, lon=-74.0, routes=["A"],
        ))
        _assert_type(data, "id", str)
        _assert_type(data, "name", str)
        _assert_type(data, "lat", (int, float))
...
class TestAllSubwayStationsResponseContract:
    ...
    def test_structure(self):
        data = _json(AllSubwayStationsResponse(
            stations=[
                SubwayStation(id="A27", name="59 St", lat=40.768, lon=-73.981, routes=["A"]),
            ],
        ))
...
class TestProcessedStationContract:
    ...
    def _make(self) -> dict:
        return _json(ProcessedStationsResponse(
            stations=[
                ProcessedStation(
                    station_id="A27",
...
class TestCommuterRailLineOverlayContract:
    ...
    def _make(self) -> dict:
        return _json(CommuterRailLineOverlay(
            route_id="LIRR_10",
            name="Babylon",
            color_hex="#00985F",
            polylines=["_p~iF~ps|U_ulLnnqC"],
            mode="lirr",
            stops=[
                CommuterRailStop(stop_id="1", name="Jamaica", lat=40.699, lon=-73.808),
            ],
        ))
    ...
    def test_defaults_explicit(self):
        data = _json(CommuterRailLineOverlay(
            route_id="X", name="N", color_hex="#C",
            polylines=[], mode="mnr",
        ))
        assert data["stops"] == []
...
class TestAllCommuterRailLinesResponseContract:
    ...
    def test_structure(self):
        data = _json(AllCommuterRailLinesResponse(
            lines=[
                CommuterRailLineOverlay(
                    route_id="LIRR_10", name="Babylon",
                    color_hex="#00985F", polylines=["abc"], mode="lirr",
                ),
            ],
        ))
...
class TestBusScheduleContract:
    ...
    def _make(self) -> dict:
        return _json(BusScheduleResponse(
            route_id="MTA NYCT_B63",
            directions=[
                BusScheduleDirection(
                    route_id="MTA NYCT_B63",
...
class TestDelayPredictionContract:
    ...
    IOS_KEYS = [
        "adjusted_minutes", "original_minutes", "delay_factor",
        "adjustment_reason", "model_source", "recency_error_seconds",
    ]
...
class TestSnakeCaseConsistency:
        ...
        if isinstance(data, list):
            ...
            for key, value in data.items():
                ...
                if len(key) > 1 and any(c.isupper() for c in key[1:]):
                    assert False, f"CamelCase key found at {path}.{key}"
    ...
    def test_bus_vehicle_snake_case(self):
        data = _json(BusVehicle(
            vehicle_id="V", route_id="R", lat=0, lon=0,
            onward_calls=[
                BusArrival(route_id="R", vehicle_id="V", stop_id="S", status_text="T"),
            ],
        ))
    ...
    def test_all_subway_lines_response_snake_case(self):
        data = _json(AllSubwayLinesResponse(
            ...
            trunk_polylines=[
                TrunkGroupPolylines(
                    trunk_index=0, color_hex="#C", route_ids=["A"],
                    polylines=["abc"], lane_offset=-12.0,
    ...
    def test_route_shape_snake_case(self):
        data = _json(RouteShape(
            route_id="L", polylines=["abc"],
            stops=[BusStop(id="S", name="N", lat=0, lon=0)],
            directions=[
                DirectionShape(
                    direction_id=0, headsign="H",
                    polylines=["abc"],
                    stops=[BusStop(id="S", name="N", lat=0, lon=0)],
                ),
            ],
    ...
    def test_processed_stations_snake_case(self):
        data = _json(ProcessedStationsResponse(
            stations=[
                ProcessedStation(
                    station_id="A27", name="59 St",
                    is_transfer=True,
...
class TestFullRoundtrip:
    ...
    def test_complete_grouped_nearby_response(self):
        ...
        response = [
            GroupedNearbyTransit(
                    ...
                    DirectionArrivals(
                        ...
                        arrivals=[
                            NearbyTransitArrival(
                                ...
                                minutes_away=3, arrival_ts=ts,
                                status="On Time", mode="subway",
                                stop_lat=40.71, stop_lon=-74.0,
                                stop_id="A28N", trip_id="trip_1",
                                is_real_time=True,
                            ),
                        ],
                    ),
                ...
                alerts=[
                    InlineAlert(
                        title="Delays", severity="severe",
                        affected_routes=["A"], alert_type="Delays",
                        sort_order=26,
                    ),
                ],
            ),
                    ...
                    DirectionArrivals(
                        ...
                        arrivals=[
                            NearbyTransitArrival(
                                ...
                                minutes_away=7, arrival_ts=ts + 240,
                                status="2 stops away", mode="bus",
                                stop_lat=40.68, stop_lon=-73.97,
                                stop_id="MTA_300456",
                                vehicle_id="MTABC_5678",
                                distance_m=400.0,
                                is_real_time=True,
        ...
        for group in response:
            data = _json(group)
            _assert_keys(data, TestGroupedNearbyTransitContract.IOS_KEYS, f"group {data['route_id']}")
            for direction in data["directions"]:
                _assert_keys(direction, TestGroupedNearbyTransitContract.DIRECTION_KEYS, "direction")
                for arrival in direction["arrivals"]:
                    _assert_keys(arrival, TestNearbyTransitArrivalContract.IOS_KEYS, "arrival")
            for alert in data.get("alerts", []):
                _assert_keys(alert, TestGroupedNearbyTransitContract.ALERT_KEYS, "alert")

    def test_complete_system_map_response(self):
        """Simulate a full /subway/shapes/all response."""
        data = _json(AllSubwayLinesResponse(
            lines=[
                SubwayLineOverlay(route_id="A", color_hex="#0039A6", polylines=["abc"]),
                SubwayLineOverlay(route_id="L", color_hex="#A7A9AC", polylines=["def"]),
            ],
            trunk_polylines=[
                TrunkGroupPolylines(
                    trunk_index=0, color_hex="#0039A6",
    ...
    def test_complete_commuter_rail_response(self):
        ...
        _assert_keys(line["stops"][0], TestCommuterRailLineOverlayContract.STOP_KEYS, "stop")
